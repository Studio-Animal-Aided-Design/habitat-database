...
export async function verifyScryptSecret(secret: string, encodedHash: string): Promise<boolean> {
  ...
  try {
    ...
    const actual = await new Promise<Buffer>((resolve, reject) => {
      scrypt(secret, decode(saltValue), expected.length, (error, derivedKey) => {
